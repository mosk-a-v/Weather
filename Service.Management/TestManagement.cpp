...
void TestManageBoiler() {
    TestManagement *testManagement = IninManagement();
    testManagement->SetBoilerResponseTime(0);
    if(testManagement->GetBoilerTemperature() != 35) throw new exception();
    ...
    if(testManagement->GetBoilerStatus() != true) throw new exception();
...
TestManagement* IninManagement() {
    ...
    return new TestManagement(control, setting);
}
...
TestManagement::TestManagement(const std::vector<ControlValue>& controlTable, const std::vector<SettingValue>& settingsTable) :
