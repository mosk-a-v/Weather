...
CurrentWeather* GlobalWeather::GetWeather() {
    ...
    try {
        ...
        std::stringstream ss;
        ss << e.what() << GlobalWeatherResponse;
        Utils::WriteLogInfo(LOG_WARNING, "Openweathermap responce read exception.", ss.str());
    }
    return weather;
}
bool GlobalWeather::DownloadJSON(std::string URL) {
    ...
    if(curl) {
        ...
        if(CURLE_OK == res) {
            ...
            std::stringstream ss;
            ss << res;
            Utils::WriteLogInfo(LOG_WARNING, "Curl error", ss.str());
            result = false;
        }
